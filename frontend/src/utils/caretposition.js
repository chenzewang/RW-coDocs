	...
	this.caretPos = function (textarea, mode) {
		...
		var getCaretPosition = function (element) {
			...
			if (document.selection) {
				...
				var range_end = textRange.duplicate();
				range_end.setEndPoint('EndToEnd', docRange);
				endpos = range_end.text.length;
		...
		var getElementPosition = function (element) {
			...
			var rect = element.getBoundingClientRect(); //相对于视窗的位置
		...
		* Main function start here! *
		...
		switch (mode) {
			case 'self':
				...
				return {
					left: caretPos.left - dummyTextAreaPos.left, top: caretPos.top - dummyTextAreaPos.top
				};
			case 'body':
			case 'screen':
			case 'stage':
			...
			default:
				...
				return {
					left: textAreaPosition.left + caretPos.left - dummyTextAreaPos.left, top: textAreaPosition.top + caretPos.top - dummyTextAreaPos.top
				};
		}
	};
};
