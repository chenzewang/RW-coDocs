<template>
  <div>
    ...
    <div>
      ...
      <div v-else>
        <a-button type="danger" block style="margin-bottom=10px" @click="Quit()"
          >退出团队</a-button
        >
      </div>
    </div>
...
export default {
    ...
    groupid: {
      handler(newVal) {
        if (localStorage.getItem("groupInfo" + newVal)) {
          this.groupObj = JSON.parse(
            localStorage.getItem("groupInfo" + newVal)
          );
        } else {
        ...
        axios
          .post(
            "http://localhost:5000/api/groupiscreatedbyme/",
            formData,
            config
          )
  ...
  methods: {
    ...
    get_groupmem() {
      ...
      var _this = this;
      axios
        .post("http://localhost:5000/api/get_user_bygroup/", formData, config)
        .then(function(response) {
          if (response) {
            _this.data2 = response.data;
            _this.data2 = _this.data2.filter(
              (item) => item.username != localStorage.getItem("token")
            );
          }
        })
        .catch((error) => {
    ...
    deleteDocs(id) {
      ...
      this.$confirm({
        ...
        onOk() {
          ...
          axios
            .post("http://localhost:5000/api/recycle_doc_2/", formData, config)
            .then(function(response) {
    ...
    Quit() {
      ...
      this.$confirm({
        ...
        onOk() {
          ...
          axios
            .post("http://localhost:5000/api/quit_group/", formData, config)
            .then(function(response) {
              console.log("返回的结果是" + response.data.message);
              if (response.data.message == "success") {
