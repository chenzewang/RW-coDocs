        ...
        <a-layout style="padding: 0 24px 24px">
          ...
          <a-row style="margin-bottom:10px;">
            ...
            >
              <memberAvatar
                :username="user.name"
                :style="'float:right;background-color:' + randomColor()"
              ></memberAvatar>
            </span>
          </a-row>
          <a-layout-content
          ...
          >
            <editor
              v-if="watch_right || modify_right"
              v-on:userListChange="userListHandle"
              :userList="userList"
            ></editor>
          </a-layout-content>
          <a-button @click="getPdf()"
            ><a-icon type="download" size="small" />导出为pdf</a-button
          >
          <a-button @click="exportWord()"
            ><a-icon type="download" />导出为word</a-button
          >
        </a-layout>
      </a-layout>
    </a-layout>
...
export default {
  ...
  methods: {
    ...
    initWebSocket() {
      //初始化weosocket
      const wsuri = "ws://localhost:8888/conn";
